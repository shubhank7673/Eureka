...
const transporter = nodemailer.createTransport(sendGridTransport({
    auth:{
        api_key: 'api_key_is_hidden'
    }
}));

exports.getSettings = (req, res, next) => {
    res.render('students/settings/settings',{ pageTitle: 'Settings' });
};
...
exports.getChangeName = (req, res, next) => {
    ...
    res.render('students/settings/changeName', {
...
exports.getChangePassword = (req, res, next) => {
    res.render('students/settings/changePassword', { pageTitle: 'Change Password' });
};
    ...
    bcrypt.compare(oldPassword, req.student.password).then(doMatch => {
        if (doMatch) {
            bcrypt.hash(newPassword, 12).then(password => {
                ...
                req.student.save().then(() => {
                    res.render('students/settings/settings');
                });
            }).catch(err => console.log(err));
        } else {
            res.render('students/settings/changePassword');
        }
    }).catch(err => console.log(err));
};

exports.getFeedback = (req, res, next) => {
    res.render('students/settings/feedback', { pageTitle: 'Feedback' });
};
